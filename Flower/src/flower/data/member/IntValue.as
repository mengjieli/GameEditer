package flower.data.member
{
	public class IntValue extends Value
	{
		...
		public function IntValue(initValue:*=0)
		{
			if(initValue) {
		...
		private static var pool:Vector.<IntValue> = new Vector.<IntValue>();
		public static function create(initValue:*=null):IntValue {
			var value:IntValue;
			if(pool.length) {
				value = pool.pop();
				value._events = [];
				...
				value = new IntValue(initValue);
			}
			return value;
		}
		
		public static function release(value:IntValue):void {
			value.dispose();
			pool.push(value);
		}
	}
