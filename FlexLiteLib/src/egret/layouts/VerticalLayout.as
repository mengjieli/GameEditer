package egret.layouts
{
	...
	public class VerticalLayout extends LayoutBase
	{
		...
		private function measureVirtual():void
		{
			...
			{
				target.measuredMinWidth = (horizontalAlign == HorizontalAlign.JUSTIFY) ? hPadding : target.measuredWidth;
				target.measuredMinHeight = Math.ceil(target.measuredHeight);
			}
		}
		...
		override public function getNavigationDestinationIndex(currentIndex:int, navigationUnit:uint):int
		{
			...
			switch (navigationUnit)
			{
				...
				case NavigationUnit.PAGE_UP:
					var firstIndex:int = findIndexAt(target.scrollRect.top,0,target.numElements-1);
					var lastIndex:int = findIndexAt(target.scrollRect.bottom,0,target.numElements-1);
				...
				case NavigationUnit.PAGE_DOWN:
					firstIndex = findIndexAt(target.scrollRect.top,0,target.numElements-1);
					lastIndex = findIndexAt(target.scrollRect.bottom,0,target.numElements-1);
		...
		public function fractionOfElementInView(index:int):Number 
		{
			...
			if (useVirtualLayout)
			{
				eltY = getStartPosition(index);
				eltHeight = getElementSize(index);
			}
			else 
			{
		...
		private static var cacheHeightDic:Dictionary = new Dictionary(true);
		/**
		 * 更新使用真实布局的显示列表
		 */		
		private function updateDisplayListReal(width:Number,height:Number):void
		{
			...
			var heightDic:Dictionary = cacheHeightDic;
			if(vJustify)
			{
				if(excessSpace<0)
				{
		...
		override public function getElementBounds(index:int):Rectangle
		{
			...
			bounds.top = getStartPosition(index);
			bounds.bottom = getElementSize(index)+bounds.top;
			return bounds;
		}
		...
		private function getElementWidth(element:ILayoutElement, justify:Boolean, result:SizesAndLimit):void
		{
			...
			var elementPreferredWidth:Number = Math.ceil(element.preferredWidth);
		...
		private function getElementHeight(element:ILayoutElement, fixedRowHeight:Number, result:SizesAndLimit):void
		{
			...
			var elementPreferredHeight:Number = isNaN(fixedRowHeight) ? Math.ceil(element.preferredHeight) :
				fixedRowHeight;
