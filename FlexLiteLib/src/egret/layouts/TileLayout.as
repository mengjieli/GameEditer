	...
	public class TileLayout extends LayoutBase
	{
		...
		override public function measure():void
		{
			...
			if (columnCount > 0)
			{
				...
				if(!UIGlobals.getForEgret(this))
					measuredMinWidth = _columnCount * (_columnWidth + _horizontalGap) - _horizontalGap;
			}
			...
			if (rowCount > 0)
			{
				...
				if(!UIGlobals.getForEgret(this))
					measuredMinHeight = _rowCount * (_rowHeight + _verticalGap) - _verticalGap;
			}
			...
			//			if(!UIGlobals.forEgret)
			//			target.measuredMinWidth = Math.ceil(measuredMinWidth + hPadding);
			target.measuredHeight = Math.ceil(measuredHeight + vPadding);
			//			if(!UIGlobals.forEgret)
			//			target.measuredMinHeight = Math.ceil(measuredMinHeight + vPadding);
												...
												cellWidth:int,cellHeight:int):void
		{
			...
			if(!UIGlobals.getForEgret(this))
			{
				...
				elementWidth = Math.max(element.getMinBoundsWidth(), Math.min(maxChildWidth, elementWidth));
				elementHeight = Math.max(element.getMinBoundsHeight(), Math.min(maxChildHeight, elementHeight));
			}
