package egret.components
{
	...
	import egret.layouts.TileLayout;
	import egret.layouts.TileOrientation;
	...
	public class PageNavigator extends SkinnableComponent implements IVisualElementContainer
	{
		...
		private function startAnimation(valueFrom:Number,valueTo:Number):void
		{
			...
			animator.motionPaths = new <MotionPath>[
				new SimpleMotionPath("scrollPosition", valueFrom, valueTo)];
			animator.play();
		}
		 ...
		 * 在属性提交前缓存外部显式设置的页码值
		 */
		ns_egret var proposedCurrentPage:int = NO_PROPOSED_PAGE;
		...
		private function installViewport():void
		{
			if (contentGroup&&_viewport)
			{
				...
				_viewport.addEventListener(ResizeEvent.UI_RESIZE,onViewPortResized);
		...
		private function uninstallViewport():void
		{
			if (_skin && _viewport)
			{
				...
				_viewport.removeEventListener(ResizeEvent.UI_RESIZE,onViewPortResized);
				if(_viewport is GroupBase)
				{
		 ...
		 */		
		ns_egret function updateDirection():Boolean
		{
			if(!(_viewport is GroupBase))
				return true;
			var layout:LayoutBase = (_viewport as GroupBase).layout;
