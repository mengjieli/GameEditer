	...
	public class ProgressBar extends Range
	{
		...
		override public function set value(newValue:Number):void
		{
			...
			else
			{
				...
				{
					var valueTo:Number = Number(SimpleMotionPath(animator.motionPaths[0]).valueTo);
					setValue(nearestValidValue(valueTo , snapInterval));
					animator.stop();
				}
				...
				animator.motionPaths = new <MotionPath>[
					new SimpleMotionPath("value", super.value, slideToValue)];
				animator.play();
			}
		}
		...
		override protected function partAdded(partName:String, instance:Object):void
		{
			if(instance==track)
			{
				...
				{
					track.addEventListener(ResizeEvent.UI_RESIZE,onTrackResizeOrMove);
		...
		override protected function partRemoved(partName:String, instance:Object):void
		{
			if(instance==track)
			{
				if(track is UIComponent)
				{
					track.removeEventListener(ResizeEvent.UI_RESIZE,onTrackResizeOrMove);
