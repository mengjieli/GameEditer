package egret.components
{
	 ...
	 */	
	[Event(name="playComplete", type="egret.events.UIEvent")]
	/**
	...
	public class UIMovieClip extends UIAsset implements IMovieClip
	{
		 ...
		 * @inheritDoc
		 */	
		override protected function onGetContent(content:Object, source:Object):void
		{
			super.onGetContent(content,source);
			if(movieClip)
				detachMovieClip(movieClip);
			isMovieClip = (content is MovieClip);
			if(isMovieClip||content is IMovieClip)
			{
				movieClip = content;
			}
			else
