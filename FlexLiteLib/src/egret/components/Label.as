package egret.components
{
	...
	public class Label extends TextBase
	{
		public function Label()
		{
			...
			if(GlobalStyles.embedFontFamily)
			{
				fontFamily = GlobalStyles.embedFontFamily;
			}
			cacheAsBitmap = true;
		...
		private var lastScreenDPI:Number = 2;
		/**
		 * 屏幕DPI改变事件
		 */		
		private function onScreenDPIChanged(event:Event=null):void
		{
			if(lastScreenDPI==stage.contentsScaleFactor)
				return;
			lastScreenDPI = stage.contentsScaleFactor;
			if(lastScreenDPI==1)
			{
				cacheAsBitmap = false;
			}
		...
		/**
		 * 是否只显示嵌入的字体。此属性对只所有Label实例有效,对输入文本无效。true表示如果指定的fontFamily没有被嵌入，
		...
		override public function styleChanged(styleProp:String):void
		{
			...
			if(!styleProp||styleProp=="verticalAlign")
			{
				if(textField)
		...
		override ns_egret function get realLeading():int
		{
			return verticalAlign==VerticalAlign.JUSTIFY?0:leading;
		}
		 ...
		 * leading，letterSpacing,verticalAlign属性。
		 */	
		...
		private function measureUsingWidth(w:Number):void
		{
			...
			textField.wordWrap = (_maxDisplayedLines!=1);
		 ...
		 * 文字内容发生改变
		 */		
		override ns_egret function textField_textModifiedHandler(event:Event):void
		{
