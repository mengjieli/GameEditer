package egret.components
{
	...
	public class PopUpAnchor extends UIComponent
	{
		...
		public function set popUp(value:IVisualElement):void
		{
			...
			_popUp = value;
			_popUp.addEventListener(MouseEvent.MOUSE_DOWN,mouseDownHandler,false,-50);
		...
		private var _popUpPositionAdjust:String = PopUpPosition.TOP_LEFT;
		/**
		...
		private function calculatePopUpPosition():Point
		{
			...
			_popUpPositionAdjust = _popUpPosition;
			...
			if(registrationPoint.y+_popUp.layoutBoundsHeight > stage.stageHeight && _popUpPosition == PopUpPosition.BELOW)
			{
				_popUpPositionAdjust = PopUpPosition.ABOVE;
				registrationPoint.x = 0;
				registrationPoint.y = -popUp.layoutBoundsHeight;
				registrationPoint = localToGlobal(registrationPoint);
				registrationPoint = popUp.parent.globalToLocal(registrationPoint);
			}
			if(registrationPoint.x+_popUp.layoutBoundsWidth > stage.stageWidth && _popUpPosition == PopUpPosition.BELOW)
			{
				_popUpPositionAdjust = PopUpPosition.BELOW;
				registrationPoint.y = height;
				registrationPoint = localToGlobal(registrationPoint);
				registrationPoint.x = stage.stageWidth-popUp.layoutBoundsWidth;
				registrationPoint = popUp.parent.globalToLocal(registrationPoint);
				
			}
			if(registrationPoint.y-_popUp.layoutBoundsHeight < 0 && _popUpPosition == PopUpPosition.ABOVE)
			{
				_popUpPositionAdjust = PopUpPosition.BELOW;
				registrationPoint.x = 0;
				registrationPoint.y = height;
			...
			if(registrationPoint.x+_popUp.layoutBoundsWidth > stage.stageWidth && _popUpPosition == PopUpPosition.RIGHT)
			{
				_popUpPositionAdjust = PopUpPosition.LEFT;
				registrationPoint.x = -popUp.layoutBoundsWidth;
				registrationPoint = localToGlobal(registrationPoint);
				registrationPoint = popUp.parent.globalToLocal(registrationPoint);
				registrationPoint.y = 0;
			}
			if(registrationPoint.x-_popUp.layoutBoundsWidth < 0 && _popUpPosition == PopUpPosition.LEFT)
			{
				_popUpPositionAdjust = PopUpPosition.RIGHT;
				registrationPoint.x = width;
				registrationPoint.y = 0;
				registrationPoint = localToGlobal(registrationPoint);
				registrationPoint = popUp.parent.globalToLocal(registrationPoint);
		...
		private function createMotionPath():Vector.<MotionPath>
		{
			...
			if(!popUpIsDisplayed)
			{
				var tempValue:Number = Number(xPath.valueFrom);
				xPath.valueFrom = xPath.valueTo;
				xPath.valueTo = tempValue;
				tempValue = Number(yPath.valueFrom);
