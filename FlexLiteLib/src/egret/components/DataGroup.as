package egret.components
{
	...
	import egret.core.ISkinnableClient;
	 ...
	 * 添加了项呈示器 
	 */	
	[Event(name="rendererAdd", type="egret.events.RendererExistenceEvent")]
	/**
	 * 移除了项呈示器 
	 */	
	[Event(name="rendererRemove", type="egret.events.RendererExistenceEvent")]
	...
	public class DataGroup extends GroupBase
	{
		 ...
		 * 项呈示器的主机组件
		 */
		ns_egret function get rendererOwner():IItemRendererOwner
		{
			return _rendererOwner;
		}

		ns_egret function set rendererOwner(value:IItemRendererOwner):void
		{
		...
		private function setItemRenderSkinName(renderer:IItemRenderer):void
		{
			...
			if(comp)
			{
				if(!comp.skinNameExplicitlySet)
				{
					...
					comp.skinNameExplicitlySet = false;
				}
			}
			else
			{
