package egret.components
{
	 ...
	 */	
	[Event(name="itemOpening", type="egret.events.TreeEvent")]
	/**
	 * 节点打开，注意：只有通过交互操作引起的节点打开才会抛出此事件。
	 */	
	[Event(name="itemOpen", type="egret.events.TreeEvent")]
	/**
	 * 节点关闭,注意：只有通过交互操作引起的节点关闭才会抛出此事件。
	 */	
	[Event(name="itemClose", type="egret.events.TreeEvent")]
	...
	public class Tree extends List
	{
		 ...
		 * 确保某一项可见
		 */
		public function ensureItemIsVisible(item:Object):void
		{
			if(!dataGroup || !dataProvider || !item)
				return;
			if(dataProvider is ITreeCollection)
			{
				...
				while(parentNode)
				{
					ITreeCollection(dataProvider).expandItem(parentNode);
