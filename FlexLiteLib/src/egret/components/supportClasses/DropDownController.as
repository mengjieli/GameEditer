package egret.components.supportClasses
{
	...
	public class DropDownController extends EventDispatcher
	{
		...
		{
			return _openButton?_openButton.stage:null;
		}
		/**
		...
		private function addCloseTriggers():void
		{
			...
			if (stage)
			{
				if (isNaN(rollOverOpenDelay))
				{
					...
					stage.addEventListener(MouseEvent.MOUSE_UP, stage_mouseUpHandler_noRollOverOpenDelay);
				}
				else
				{
		...
		private function removeCloseTriggers():void
		{
			...
			if (stage)
			{
				if (isNaN(rollOverOpenDelay))
				{
					...
					stage.removeEventListener(MouseEvent.MOUSE_UP, stage_mouseUpHandler_noRollOverOpenDelay);
				}
				else
				{
		 ...
		 * openButton上按下鼠标事件
		 */		
		ns_egret function openButton_buttonDownHandler(event:Event):void
		{
			if (isOpen)
				closeDropDown(true);
			else
		 ...
		 * openButton上鼠标经过事件
		 */		
		ns_egret function openButton_rollOverHandler(event:MouseEvent):void
		{
		 ...
		 * 舞台上鼠标按下事件
		 */		
		ns_egret function stage_mouseDownHandler(event:Event):void
		{
		 ...
		 * 舞台上鼠标移动事件
		 */		
		ns_egret function stage_mouseMoveHandler(event:Event):void
		{
			var target:DisplayObject = event.target as DisplayObject;
		 ...
		 * 舞台上鼠标弹起事件
		 */		
		ns_egret function stage_mouseUpHandler_noRollOverOpenDelay(event:Event):void
		{
		 ...
		 * 舞台上鼠标弹起事件
		 */	
		ns_egret function stage_mouseUpHandler(event:Event):void
		{
			var target:DisplayObject = event.target as DisplayObject;
			var containedTarget:Boolean = isTargetOverDropDownOrOpenButton(target);
			if (containedTarget)
		 ...
		 * 舞台尺寸改变事件
		 */		
		ns_egret function stage_resizeHandler(event:Event):void
		{
			closeDropDown(true);
		}    
		/**
