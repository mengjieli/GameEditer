package egret.components.supportClasses
{
	...
	import egret.components.SkinnableDataContainer;
	 ...
	 */	
	[Event(name="itemRollOver", type="egret.events.ListEvent")]
	/**
	 * 指示用户执行了将鼠标指针从控件中某个项呈示器上移开的操作  
	 */	
	[Event(name="itemRollOut", type="egret.events.ListEvent")]
	/**
	 * 指示用户执行了将鼠标在某个项呈示器上单击的操作。 
	 */	
	[Event(name="itemClick", type="egret.events.ListEvent")]
	/**
	...
	public class ListBase extends SkinnableDataContainer
	{
		 ...
		 * 自定义的选中项
		 */		
		ns_egret static var CUSTOM_SELECTED_ITEM:int = -3;
		 ...
		 * 正在进行所有数据源的刷新操作
		 */		
		ns_egret var doingWholesaleChanges:Boolean = false;
		 ...
		 * 在属性提交前缓存真实的选中项的值
		 */
		ns_egret var _proposedSelectedIndex:int = NO_PROPOSED_SELECTION;
		
		ns_egret var _selectedIndex:int = NO_SELECTION;
		 ...
		 * 是否允许自定义的选中项
		 */		
		ns_egret var allowCustomSelectedItem:Boolean = false;
		/**
		 * 索引改变后是否需要抛出事件 
		 */		
		ns_egret var dispatchChangeAfterSelection:Boolean = false;
		
		/**
		 * 设置选中项
		 */
		ns_egret function setSelectedIndex(value:int, dispatchChangeEvent:Boolean = false):void
		{
		 ...
		 *  在属性提交前缓存真实选中项的数据源
		 */
		ns_egret var _pendingSelectedItem:*;
		 ...
		 * 设置选中项数据源
		 */
		ns_egret function setSelectedItem(value:*, dispatchChangeEvent:Boolean = false):void
		{
		 ...
		 * 返回指定索引是否等于当前选中索引
		 * 
		 * 此处改为判断索引是否为已经验证过的当前选中索引。 防止updaterender强行将为进行验证的render设置selected为true;
		 */
		ns_egret function isItemIndexSelected(index:int):Boolean
		{        
		...
		ns_egret var selectedIndexAdjusted:Boolean = false;
		/**
		 ...
		 */		
		ns_egret function dispatchListEvent(mouseEvent:MouseEvent,type:String,itemRenderer:IItemRenderer):void
		{
			var itemIndex:int = -1;
			if (itemRenderer)
