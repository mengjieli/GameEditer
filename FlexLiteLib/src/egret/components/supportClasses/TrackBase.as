package egret.components.supportClasses
{
	 ...
	 * 改变结束
	 */	
	[Event(name="changeEnd", type="egret.events.UIEvent")]
	/**
	 * 改变开始
	 */	
	[Event(name="changeStart", type="egret.events.UIEvent")]
	
	/**
	 * 按下滑块并使用鼠标移动滑块时分派。此事件始终发生在 thumbPress 事件之后。
	 */	
	[Event(name="thumbDrag", type="egret.events.TrackBaseEvent")]
	
	/**
	 * 按下滑块（即用户在滑块上按下鼠标按钮）时分派。
	 */	
	[Event(name="thumbPress", type="egret.events.TrackBaseEvent")]
	
	/**
	 * 放开滑块（即用户在滑块上弹起鼠标按钮）时分派。
	 */	
	[Event(name="thumbRelease", type="egret.events.TrackBaseEvent")]
	...
	public class TrackBase extends Range
	{
		...
		override protected function partAdded(partName:String, instance:Object):void
		{
			...
			if (instance == thumb)
			{
				...
				thumb.addEventListener(ResizeEvent.UI_RESIZE, thumb_resizeHandler);
			...
			else if (instance == track)
			{
				...
				track.addEventListener(ResizeEvent.UI_RESIZE, track_resizeHandler);
			}
		}
		...
		override protected function partRemoved(partName:String, instance:Object):void
		{
			...
			if (instance == thumb)
			{
				...
				thumb.removeEventListener(ResizeEvent.UI_RESIZE, thumb_resizeHandler);            
			...
			else if (instance == track)
			{
				...
				track.removeEventListener(ResizeEvent.UI_RESIZE, track_resizeHandler);
			}
		}
		 ...
		 * 记录鼠标在thumb上按下的位置
		 */		
		ns_egret var clickOffset:Point;
		...
		protected function updateWhenMouseMove():void
		{
			if(!track || !stage)
				return;
			var p:Point = track.globalToLocal(new Point(this.stage.mouseX, this.stage.mouseY));
		...
		{
			Stage(event.currentTarget).removeEventListener(MouseEvent.MOUSE_MOVE, 
				stage_mouseMoveHandler);
			Stage(event.currentTarget).removeEventListener(MouseEvent.MOUSE_UP, 
				stage_mouseUpHandler);
			Stage(event.currentTarget).removeEventListener(Event.MOUSE_LEAVE,
				stage_mouseUpHandler);
