package egret.collections
{
	 ...
	 * @author dom
	 */
	public class ArrayCollection extends Proxy implements ICollectionView,ICollection
	{
		/**
		 * 构造函数
		 * @param source 数据源
		...
		public function sort(compareFunction:Function=null):void
		{
			if(compareFunction!=null)
			{
				...
				if(filterSource)
					filterSource.sort(compareFunction);
			}
			else
			{
				_source.sort();
		 ...
		 * @inheritDoc
		 */
		public function sortOn(fieldName:String,options:*):void
		{
			_source.sortOn(fieldName,options);
			if(filterSource)
				filterSource.sortOn(fieldName,options);
			refresh();
		}
		
		private var filterSource:Array;
		/**
		 * 获取实际的数据源
		 */		
		private function getSource():Array
		{
			return filterSource?filterSource:_source;
		}
		...
		/**
		 * 在对数据源source进行排序或过滤等修改操作后可以手动调用此方法刷新所有数据,以更新视图。
		 */		
		...
		public function addItemAt(item:Object, index:int):void
		{
			...
			if(_filterFunction!=null)
			{
				var oldItem:Object = filterSource[index];
				var sourceIndex:int;
				if(oldItem)
				{
					sourceIndex = _source.indexOf(oldItem);
				}
				else
				{
					sourceIndex = _source.length;
				}
				_source.splice(sourceIndex,0,item);
				if(_filterFunction(item))
				{
					filterSource.splice(index,0,item);
		...
		public function removeItemAt(index:int):Object
		{
			...
			if(filterSource)
			{
				var sourceIndex:int = _source.indexOf(item);
				if(sourceIndex!=-1)
					_source.splice(sourceIndex,1);
			}
		...
		public function replaceItemAt(item:Object, index:int):Object
		{
			...
			if(filterSource)
			{
				var sourceIndex:int = _source.indexOf(oldItem);
				if(sourceIndex!=-1)
				{
					_source.splice(sourceIndex,1,item);
				}
			}
		...
		public function moveItemAt(oldIndex:int,newIndex:int):Object
		{
			...
			checkIndex(newIndex);
			var source:Array = getSource();
			var item:Object = source.splice(oldIndex,1)[0];
			var newItem:Object = source[newIndex];
			...
			if(filterSource)
			{
				...
				{
					_source.splice(sourceIndex,1);
					sourceIndex = _source.indexOf(newItem);
					_source.splice(sourceIndex,0,item);
				}
			}
