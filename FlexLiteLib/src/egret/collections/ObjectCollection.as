package egret.collections
{
		...
		implements ICollection,ITreeCollection
	{
		 ...
		 * 节点是否显示
		 */
		private function isShow(item:Object):Boolean
		{
		...
		{
			if(!isShow(parent) || !parent.hasOwnProperty(childrenKey)||_openNodes.indexOf(parent)==-1)
				return;
		...
		public function hasChildren(item:Object):Boolean
		{
			if(item.hasOwnProperty(childrenKey))
			{
				...
				{
					var isChildShow:Boolean;
					for (var i:int = 0; i < item[childrenKey].length; i++) 
					{
						var child:Object = item[childrenKey][i];
						if(isShow(child))
						{
							isChildShow = true;
							break;
		...
		{
			if(_openNodes.indexOf(item)==-1 && isShow(item))
			{
