package egret.managers
{
	...
	import locales.locale_egret_desktop;
	...
	public class SystemManager extends Group implements ISystemManager
	{
		...
		//强制让多语言配置被编译
		private var locale:locale_egret_desktop;
		/**
		 * 添加到舞台
		 */		
		private function onAddToStage(event:Event=null):void
		{
			...
			UIGlobals.addSystemManager(this);
			if(_autoResize)
			{
		...
		{
			UIGlobals.removeSystemManager(this);
			if(_autoResize)
			{
		...
		public function get popUpContainer():IContainer
		{
			if (!_popUpContainer)
			{
				_popUpContainer = new SystemContainer(this,
					new QName(ns_egret, "noTopMostIndex"),
					new QName(ns_egret, "topMostIndex"));
			}
		...
		public function get toolTipContainer():IContainer
		{
			if (!_toolTipContainer)
			{
				_toolTipContainer = new SystemContainer(this,
					new QName(ns_egret, "topMostIndex"),
					new QName(ns_egret, "toolTipIndex"));
			}
		...
		public function get cursorContainer():IContainer
		{
			if (!_cursorContainer)
			{
				_cursorContainer = new SystemContainer(this,
					new QName(ns_egret, "toolTipIndex"),
					new QName(ns_egret, "cursorIndex"));
			}
		...
		ns_egret function set noTopMostIndex(value:int):void
		{
		...
		ns_egret function set topMostIndex(value:int):void
		{
		...
		ns_egret function set toolTipIndex(value:int):void
		{
		...
		ns_egret function set cursorIndex(value:int):void
		{
		...
		//                                保留容器原始操作方法
		//==========================================================================
		ns_egret function get raw_numElements():int
		{
			return super.numElements;
		}
		ns_egret function raw_getElementAt(index:int):IVisualElement
		{
			return super.getElementAt(index);
		}
		ns_egret function raw_addElement(element:IVisualElement):IVisualElement
		{
		...
		ns_egret function raw_addElementAt(element:IVisualElement, index:int):IVisualElement
		{
			if (element.parent==this)
			{
				var oldIndex:int = getElementIndex(element);
		...
		ns_egret function raw_removeElement(element:IVisualElement):IVisualElement
		{
			return super.removeElementAt(super.getElementIndex(element));
		}
		ns_egret function raw_removeElementAt(index:int):IVisualElement
		{
			return super.removeElementAt(index);
		}
		ns_egret function raw_removeAllElements():void
		{
		...
		ns_egret function raw_getElementIndex(element:IVisualElement):int
		{
			return super.getElementIndex(element);
		}
		ns_egret function raw_setElementIndex(element:IVisualElement, index:int):void
		{
			super.setElementIndex(element,index);
		}
		ns_egret function raw_swapElements(element1:IVisualElement, element2:IVisualElement):void
		{
			super.swapElementsAt(super.getElementIndex(element1), super.getElementIndex(element2));
		}
		ns_egret function raw_swapElementsAt(index1:int, index2:int):void
		{
			super.swapElementsAt(index1,index2);
		}
		ns_egret function raw_containsElement(element:IVisualElement):Boolean
		{
