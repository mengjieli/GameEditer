package egret.managers.impl
{
	...
	import egret.managers.IPopUpManager;
	...
	public class PopUpManagerImpl extends EventDispatcher implements IPopUpManager
	{
		...
		{
			if(!systemManager)
				systemManager = UIGlobals.systemManagers[0];
			if(!systemManager)
				return;
		...
		public function set modalColor(value:uint):void
		{
			if(_modalColor==value)
				return;
			_modalColor = value;
			for each(var sm:ISystemManager in UIGlobals.systemManagers)
			{
				invalidateModal(sm);
			}
		}
		...
		private function invalidateModal(systemManager:ISystemManager):void
		{
			...
			if(!invalidateModalFlag)
			{
				...
				UIGlobals.stage.addEventListener(Event.ENTER_FRAME,validateModal);
				UIGlobals.stage.addEventListener(Event.RENDER,validateModal);
		...
		private function validateModal(event:Event):void
		{
			...
			UIGlobals.stage.removeEventListener(Event.ENTER_FRAME,validateModal);
			UIGlobals.stage.removeEventListener(Event.RENDER,validateModal);
