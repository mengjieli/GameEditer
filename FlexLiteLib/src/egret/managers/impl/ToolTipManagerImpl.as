package egret.managers.impl
{
	...
	import egret.components.ToolTip;
	...
	import egret.managers.IToolTipManager;
	...
	public class ToolTipManagerImpl implements IToolTipManager
	{
		...
		private function targetChanged():void
		{
			...
			if (currentTarget)
			{
				...
				if (_showDelay==0||showImmediatelyFlag||scrubTimer.running)
				{
					createTip();
					initializeTip();
					positionTip(currentToolTip,currentTarget);
					showTip();
				}
				else
				{
		...
		/**
		 * 设置ToolTip位置
		 * @param toolTip 要显示的toolTip显示对象
		 * @param toolTipClient toolTip所属的对象
		 */		
		public function positionTip(toolTip:IToolTip,toolTipClient:IToolTipManagerClient):void
		{
			if (toolTip is IInvalidating)
				IInvalidating(toolTip).validateNow();
			var x:Number;
			var y:Number;
			var sm:DisplayObjectContainer = toolTip.parent;
			var toolTipWidth:Number = toolTip.layoutBoundsWidth;
			var toolTipHeight:Number = toolTip.layoutBoundsHeight;
			var rect:Rectangle = DisplayObject(toolTipClient).getRect(sm);
			var centerX:Number = rect.left+(rect.width - toolTipWidth)*0.5;
			var centetY:Number = rect.top+(rect.height - toolTipHeight)*0.5;
			switch(toolTipClient.toolTipPosition)
			{
			...
			var offset:Point = toolTipClient.toolTipOffset;
			if(offset)
			{
				x += offset.x;
				y = offset.y;
		...
		private function showTip():void
		{
			...
			if(DisplayObject(currentTarget).stage)
				DisplayObject(currentTarget).stage.addEventListener(MouseEvent.MOUSE_DOWN,stage_mouseDownHandler);
			if (_hideDelay == 0)
			{
				hideTip();
			}
		...
		private function hideTip():void
		{
			...
			if (previousTarget)
			{
				DisplayObject(previousTarget).stage.removeEventListener(MouseEvent.MOUSE_DOWN,
