package egret.utils
{
	...
	public class AutoUpdater extends EventDispatcher
	{
		...
		{
			LoaderUtil.loadXML(serverUrl,onConfigComp);
		}
		/**
		 * 当前是否可以执行updateNow()。
		 */		
		...
		private function onConfigComp(xml:XML):void
		{
			...
			if(!remoteVersion||!app)
				return;
			var currentVersion:String = AppVersion.currentVersion;  
			var shouldUpdate:Boolean = compareVersion(currentVersion,remoteVersion);
			if(!shouldUpdate)
				return;
			LoaderUtil.loadByteArray(app,onBytesComp);
		}
