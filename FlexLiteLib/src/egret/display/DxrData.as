package egret.display
{
	...
	import egret.display.codec.DxrDrawer;
	...
	public class DxrData
	{
		 ...
		 * 帧偏移列表
		 */
		ns_egret var frameOffsetList:Array = [];
		...
		ns_egret var filterOffsetList:Array = [];
		/**
		...
		ns_egret var _scale9Grid:Rectangle;
		...
		ns_egret var _frameLabels:Array = [];
		/**
			...
			if(drawTarget)
			{
				...
				if(drawImmediately)
				{
					...
					for(var frame:int=frameList.length;frame>0;frame--)
					{
						...
						if(containsFilter)
							dxrDrawer.drawDisplayObject(mc,this,frame-1);
						else
							dxrDrawer.drawWithoutFilter(mc,this,frame-1);
					}
					this.dxrDrawer = null;
