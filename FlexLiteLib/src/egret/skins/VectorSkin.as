package egret.skins
{
	...
	public class VectorSkin extends Skin
	{
		...
		{
			return graphicsUI.graphics;
		}
		
		override public function set hostComponent(value:SkinnableComponent):void
		{
			if(super.hostComponent)
			{
				super.hostComponent.removeEventListener(ResizeEvent.UI_RESIZE,onResize);
			}
			if(value)
			{
				value.addEventListener(ResizeEvent.UI_RESIZE,onResize);
			}
		...
		{
			updateDisplayList(hostComponent.width,hostComponent.height);
		}
		
		/**
		 * 绘图对象
		 */
		private var graphicsUI:UIComponent = new UIComponent();
		override protected function createChildren():void
		{
			super.createChildren();
			graphicsUI.mouseChildren = false;
			this.addElement(graphicsUI);
		}
		 ...
		 * 文本投影滤镜
		 */		
		ns_egret static var textOverFilter:Array = [new DropShadowFilter(1,270,0,0.3,1,1)];
		/**
		 * 边框线条颜色
		 */		
		ns_egret static var borderColors:Array = [0xD4D4D4,0x518CC6,0x686868];
		/**
		 * 底线颜色
		 */		
		ns_egret static var bottomLineColors:Array = [0xBCBCBC,0x2A65A0,0x656565];
		/**
		 * 边框圆角
		 */		
		ns_egret static var cornerRadius:Number = 3;
		/**
		 * 填充颜色 
		 */		
		ns_egret static var fillColors:Array = [0xFAFAFA,0xEAEAEA,0x589ADB,0x3173B4,0x777777,0x9B9B9B];
		/**
		 * 主题颜色，应用到文本或者图标上
		 */		
		ns_egret static var themeColors:Array = [0x333333,0xFFFFFF];
		/**
		 * 绘制当前的视图状态
		 */		
		ns_egret function drawCurrentState(x:Number,y:Number,w:Number,h:Number,borderColor:uint,
